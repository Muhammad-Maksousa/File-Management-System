...
class GroupService {
    ...
    async getMyOwnGroups() {
        return await Group.findAll({ where: { [Op.and]: [{ ownerId: this.ownerId }] } ,attributes:['id','name','image','isPublic']});
    }
    ...
    async getOne(id) {
        return await Group.findOne({ where: { id: id } ,attributes:['id','name','image','isPublic']});
    }
}
